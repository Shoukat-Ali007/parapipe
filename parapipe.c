...
void *worker(void *arg) {
    ...
    for (int c = 0; c < num_cmds; c++) {
        ...
        if (pid == 0) {
            ...
            /* use shell to allow quoted args */
            execl("/bin/sh", "sh", "-c", wa->commands[c], (char *)NULL);
    ...
    while (1) {
        ...
        if (pret == 0) {
            ...
            for (int c = 0; c < num_cmds; c++) {
                if (pids[c] > 0) {
                    int st;
                    pid_t r = waitpid(pids[c], &st, WNOHANG);
                    if (r > 0) pids[c] = -1;
                    else alive = 1;
                }
            }
        ...
        if (pfd.revents & POLLIN) {
            while (1) {
                ssize_t nread = read(out_pipe[0], readbuf, sizeof(readbuf));
                if (nread > 0) {
                    char *newacc = realloc(acc, acc_len + nread + 1);
                    if (!newacc) { perror("realloc"); free(acc); acc = NULL; acc_len = 0; break; }
                    acc = newacc;
...
int main(int argc, char *argv[]) {
    ...
    while (p) {
        ...
        if (arrow) p = arrow + 2; else p = NULL;
    }
    free(cmd_copy);
